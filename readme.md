...
Tabs preceded with `-` (hyphen) indicate *dynamic tabs* generated by CFC and will be cleaned up on execution.

All other tabs will be ignored.

## Summary tabs

Column 1 contains variable names for those that you want to summarize. Each variable needs a *summary method* that is used when summarizing.

- `sum` - Add up the values in each period group
- `last` - Pick up the last value for each period group

If unspecified, `sum` will be used as the default.
